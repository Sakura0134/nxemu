...
bool TextureTICEntry::operator==(const TextureTICEntry& rhs) const
{
...
uint64_t TextureTICEntry::Address() const 
{
    return (((uint64_t)AddressHigh) << 32) | AddressLow;
}

uint32_t TextureTICEntry::Pitch() const
{
    if (HeaderVersion != TextureTICHeaderVersion_Pitch && HeaderVersion != TextureTICHeaderVersion_PitchColorKey) 
    {
...
uint32_t TextureTICEntry::Width() const
{
    if (HeaderVersion != TextureTICHeaderVersion_OneDBuffer)
    {
...
uint32_t TextureTICEntry::Height() const
{
    return HeightMinusOne + 1;
}

uint32_t TextureTICEntry::Depth() const 
{
    return DepthMinusOne + 1;
}

uint32_t TextureTICEntry::BaseLayer() const
{
    return LayerBase_0_2 | (LayerBase_3_7 << 3) | (LayerBase_8_10 << 8);
}

SurfacePixelFormat TextureTICEntry::PixelFormat(void) const
{
    if (Format == TextureFormat_B5G6R5 && RType == TextureComponentType_UNORM && GType == TextureComponentType_UNORM && BType == TextureComponentType_UNORM && AType == TextureComponentType_UNORM && SRGBConversion == 0) 
    {
        return SurfacePixelFormat_B5G6R5_UNORM;
    }
    g_Notify->BreakPoint(__FILE__, __LINE__);
    return SurfacePixelFormat_A8B8G8R8_UNORM;
}

bool TextureTICEntry::IsBlockLinear() const 
{
    return HeaderVersion == TextureTICHeaderVersion_BlockLinear || HeaderVersion == TextureTICHeaderVersion_BlockLinearColorKey;
}

bool TextureTICEntry::IsPitchLinear() const
{
    return HeaderVersion == TextureTICHeaderVersion_Pitch || HeaderVersion == TextureTICHeaderVersion_PitchColorKey;
}

bool TextureTSCEntry::operator==(const TextureTSCEntry& rhs) const
{
    return memcmp(Value, rhs.Value, sizeof(Value)) == 0;
}
...
const float * TextureTSCEntry::BorderColor() const 
{
    const float SRGB_CONVERSION_LUT[256] = 
    {
    ...
    Border[0] = SRGB_CONVERSION_LUT[SRGBBorderColorR];
    Border[1] = SRGB_CONVERSION_LUT[SRGBBorderColorG];
    Border[2] = SRGB_CONVERSION_LUT[SRGBBorderColorB];
    Border[3] = SRGB_CONVERSION_LUT[3];
    return Border;
}

float TextureTSCEntry::MinLod() const
{
    return ((float)MinLodClamp) / 256.0f;
}

float TextureTSCEntry::MaxLod() const
{
    return ((float)MaxLodClamp) / 256.0f;
}

float TextureTSCEntry::LodBias() const 
{
    uint32_t Mask = 1U << 12;
    return (float)((int32_t)((MipLodBias ^ Mask) - Mask)) / 256.0f;
}

size_t std::hash<TextureTICEntry>::operator()(const TextureTICEntry & TICEntry) const noexcept 
{
    return CityHash64((const char*)&TICEntry, sizeof(TICEntry));
}

size_t std::hash<TextureTSCEntry>::operator()(const TextureTSCEntry & TSCEntry) const noexcept
{
    return CityHash64((const char*)&TSCEntry, sizeof(TSCEntry));
}
