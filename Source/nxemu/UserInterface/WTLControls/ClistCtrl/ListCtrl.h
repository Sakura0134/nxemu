...
public:
	...
	{
#pragma warning(push)
#pragma warning(disable:4189) //warning C4189: 'pT': local variable is initialized but not referenced
        T* pT = static_cast<T*>(this);
...
#pragma warning(pop)
	...
	BOOL SetColumnFormat( int nColumn, UINT nFormat, UINT nFlags, CListArray < std::string >& aComboList )
	{
		if ( nColumn < 0 || nColumn >= GetColumnCount() ) 
			return FALSE;
		m_aColumns[ nColumn ].m_nFormat = nFormat;
	...
	BOOL SetItemFormat( int nItem, int nSubItem, UINT nFormat, UINT nFlags, CListArray < std::string >& aComboList )
	{
		ATLASSERT( FALSE ); // must be implemented in a derived class
		return FALSE;
	}
	...
	void DrawItem( CDCHandle dcPaint, int nItem, CRect& rcItem )
	{
		...
		for ( int nSubItem = 0, nColumnCount = GetColumnCount(); nSubItem < nColumnCount; rcSubItem.left = rcSubItem.right, nSubItem++ )
		{
			CListColumn listColumn;
			if ( !GetColumn( nSubItem, listColumn ) )
				break;
			
			rcSubItem.right = rcSubItem.left + listColumn.m_nWidth;
